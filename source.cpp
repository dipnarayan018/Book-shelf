...
struct PatronRecord
{								// this struct will hold Patron details
...
void UpdatePInfo()
{
	...
	cin >> PIDInput; /*getting the patron record*/
	while (MyCurrent != NULL)
	{
...
void Login()
{
	...
	else
	{
		...
		// cnt--;
		// if(cnt == 0) {
		// 	cout<<"5 times failed so can not able to login for now"<<endl;
		// 	return;
		// }
		Login();
	}
...
int main()
{
	...
	do
	{
		cerr << "\n\n       <=======>  Library Management System <=======>\n"
				"  1. Book Services       2. Patrons Services     3. Exit"
			 << endl;
		...
		case '2':
			do
			{
				cerr << "\n\n       <=======>  Patrons Service  <=======>\n"
						...
						"  6. View all patrons with active book borrowed.   7. Back"
					 << endl;
				cout << "Enter one choice: ";
				cin >> choice;
				switch (choice)
